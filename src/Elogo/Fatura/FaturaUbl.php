	...
	class FaturaUbl{
		...
		public function getir_ubl_xml($fatura_tasarimi = 'TICARIFATURA'){
			...
			try{
				...
				$konaklama          = $this->fatura->konaklama ?? 0;
				$urunler            = $this->fatura->getUrunler();

				$fatura_toplam_tutar           = round($this->fatura->fatura_toplam_tutar, 2);
				$fatura_toplam_kdv_tutar       = round($this->fatura->fatura_toplam_kdv_tutar, 2);
				$fatura_genel_toplam           = round($this->fatura->fatura_genel_toplam, 2);
				$fatura_indirim_tutari         = round($this->fatura->fatura_indirim_tutari, 2);
				$fatura_konaklama_vergi_tutari = round($this->fatura->fatura_konaklama_vergi_tutari, 2);
				...
				if(is_numeric($konaklama)){
					$TaxSubtotal   = $TaxTotal->addChild('TaxSubtotal', null, $this->xml_ns['cac']);
					$TaxableAmount = $TaxSubtotal->addChild('TaxableAmount', $fatura_toplam_tutar, $this->xml_ns['cbc']);
					$TaxableAmount->addAttribute('currencyID', $para_birimi);

					$TaxAmount2 = $TaxSubtotal->addChild('TaxAmount', $fatura_konaklama_vergi_tutari, $this->xml_ns['cbc']);
					$TaxAmount2->addAttribute('currencyID', $para_birimi);

					$TaxSubtotal->addChild('Percent', 2, $this->xml_ns['cbc']);

					$TaxCategory = $TaxSubtotal->addChild('TaxCategory', null, $this->xml_ns['cac']);

					$TaxScheme = $TaxCategory->addChild('TaxScheme', null, $this->xml_ns['cac']);
					$TaxScheme->addChild('Name', 'KONAKLAMA', $this->xml_ns['cbc']);
					$TaxScheme->addChild('TaxTypeCode', '0059', $this->xml_ns['cbc']);
				}
				...
				if($urunler != null){
					foreach($urunler as $id => $urun){
						...
						$urun_kdv_tutar      = round((($urun['urun_birim_fiyat'] * $urun_adet) - $urun_indirim_tutari) * ($urun_kdv_orani / 100), 2);
