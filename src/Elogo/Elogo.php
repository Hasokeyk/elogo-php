<?php
	...
	class Elogo{
		...
		public function dokuman_yolla($fatura_turu = 'EINVOICE', $zip_data, $alias = null){
			...
			if(!is_null($alias)){
				$eLogoParamList[] = "ALIAS=".$alias;
			}

			print_r($eLogoParamList);
			exit;
		...
		public function efatura_gonder(Fatura $fatura, BenimFirmam $benim_firmam, MusteriFirmasi $musteri_firmasi, $sadece_ubl_getir = false){
			...
			return $this->dokuman_yolla('EINVOICE', $zip_data, $musteri_firmasi->getAlias());
		}
		...
		public function earsiv_gonder(Fatura $fatura, BenimFirmam $benim_firmam, MusteriFirmasi $musteri_firmasi, $sadece_ubl_getir = false){
			...
			$zip_data = $this->create_zip($xml);
			return $this->dokuman_yolla('EARCHIVE', $zip_data, $musteri_firmasi->getAlias());
		}
