		...
		<html>
			...
			<body style="margin-left=0.6in; margin-right=0.6in; margin-top=0.79in; margin-bottom=0.79in">
						...
						<tbody>
							<tr id="lineTableTr">
								...
								<xsl:if test="//n1:Invoice/cbc:ProfileID='IHRACAT'">
									...
									<td class="lineTableTd" style="width:10.6%; font-size:8px;" align="center">
										<span style="font-weight:bold;">
											<xsl:text>GTİP İhracat</xsl:text>
										</span>
									</td>
								</xsl:if>
				...
				<table id="budgetContainerTable" width="800px">
					...
					<!--IHRACAT faturaların Navlun ve Sigorta-->
					<xsl:if test="//n1:Invoice/cbc:ProfileID = 'IHRACAT'">
						<xsl:choose>
							<xsl:when test="//n1:Invoice/cac:AllowanceCharge and //n1:Invoice/cac:AllowanceCharge/cbc:ChargeIndicator = 'true'">
								<xsl:for-each select="//n1:Invoice/cac:AllowanceCharge">
									<xsl:if test="cbc:AllowanceChargeReason = 'Navlun' or cbc:AllowanceChargeReason = 'Sigorta'">
										<tr id="budgetContainerTr" align="right">
											<td id="budgetContainerDummyTd"/>
											<td id="lineTableBudgetTd" align="right" width="270px">
												<span style="font-weight:bold;">
													<xsl:choose>
														<xsl:when test="cbc:AllowanceChargeReason = 'Navlun'">
															<xsl:text>Navlun</xsl:text>
														</xsl:when>
														<xsl:when test="cbc:AllowanceChargeReason = 'Sigorta'">
							...
							<td id="notesTableTd">
								...
								<xsl:if test="//n1:Invoice/cbc:Note">
									...
									<xsl:for-each select="//n1:Invoice/cbc:Note">
										<b>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;</b>
										<xsl:if test=". and not (contains(., 'TL') or contains(., 'kuruş'))">
											<xsl:value-of select="."/>
										</xsl:if>
										<xsl:if test=". and (contains(., 'TL') or contains(., 'kuruş')) ">
											<xsl:if test=". and not (contains(., 'Yalnız')) ">
												Yalnız
											</xsl:if>
											<xsl:value-of select="."/>
										</xsl:if>
		...
		<tr id="lineTableTr" valign="top">
			<xsl:choose>
												...
												<xsl:for-each select="./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme">
													<xsl:if test="cbc:TaxTypeCode=&apos;0015&apos; ">
														...
														<xsl:if test="../../cbc:TaxableAmount/@currencyID">
															...
															<xsl:if test="../../cbc:TaxableAmount/@currencyID != &apos;TRY&apos; ">
																<xsl:value-of select="../../cbc:TaxableAmount/@currencyID"></xsl:value-of>
															</xsl:if>
														</xsl:if>
													</xsl:if>
												</xsl:for-each>
				...
				<xsl:otherwise>
					...
					<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode/@listName='GTIP'">
						<td id="lineTableTd" align="right">
							<xsl:for-each select="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification[cbc:ItemClassificationCode/@listName = 'GTIP']">
								<xsl:value-of select="cbc:ItemClassificationCode"></xsl:value-of>
							</xsl:for-each>
						</td>
					</xsl:if>
					<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode/@listName='CPA'">
						<td id="lineTableTd" align="right">
							<xsl:for-each select="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification[cbc:ItemClassificationCode/@listName = 'CPA']">
								<xsl:value-of select="cbc:ItemClassificationCode"></xsl:value-of>
							</xsl:for-each>
						</td>
					</xsl:if>
					<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:OriginCountry/cbc:Name">
						<td id="lineTableTd" align="right">
							<xsl:for-each select="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:OriginCountry/cbc:Name">
								<xsl:apply-templates/>
							</xsl:for-each>
						</td>
					</xsl:if>
