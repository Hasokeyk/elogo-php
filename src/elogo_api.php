<?php
	...
	class elogo_api{
		...
		public function get_documents_list($start_date = null, $end_start = null, $document_list_type = 'IN', $document_type = 'EINVOICEDETAIL'){

			try{
				$start_date = $start_date??date('Y-m-d', strtotime('first day of this month'));
				$end_start = $end_start??date('Y-m-d', strtotime('last day of this month'));
		...
		private function create_invoice_xml($p = [], $iptal = false){
			...
			$Invoice->addChild('IssueDate', $p['fatura_tarihi'], $xmlNs['cbc']);
			$Invoice->addChild('IssueTime', date('H:i:s'), $xmlNs['cbc']);
			$Invoice->addChild('InvoiceTypeCode', $p['fatura_tip'], $xmlNs['cbc']);
			$Invoice->addChild('Note', $p['not'], $xmlNs['cbc']);

			$Invoice->addChild('DocumentCurrencyCode', $p['para_birimi'], $xmlNs['cbc']);
			$Invoice->addChild('TaxCurrencyCode', $p['para_birimi'], $xmlNs['cbc']);
			$Invoice->addChild('PaymentCurrencyCode', $p['para_birimi'], $xmlNs['cbc']);
			...
			if($p['profil_id'] == 'TICARIFATURA'){
				...
				$AdditionalDocumentReference->addChild('IssueDate', $p['fatura_tarihi'], $xmlNs["cbc"]);
			...
			$TaxAmount->addAttribute('currencyID', $p['para_birimi']);
			...
			$TaxableAmount->addAttribute('currencyID', $p['para_birimi']);
			$TaxAmount2 = $TaxSubtotal->addChild('TaxAmount', $p['fatura']['kdv_tutar'], $xmlNs['cbc']);
			$TaxAmount2->addAttribute('currencyID', $p['para_birimi']);
			...
			if(isset($p['fatura']['kdv_muhafiyet_kodu']) and $p['fatura']['kdv_muhafiyet_kodu'] != null){
				...
				$TaxCategory->addChild('TaxExemptionReason', $kdv_sebep[$p['fatura']['kdv_muhafiyet_kodu']], $xmlNs['cbc']);
			}
			...
			if($p['fatura']['urun_hizmet'] != null){
				foreach($p['fatura']['urun_hizmet'] as $id => $urun){
					...
					$Item->addChild('Description', $urun['hizmet_adi'], $xmlNs['cbc']);
					$Item->addChild('Name', $urun['hizmet_aciklama'], $xmlNs['cbc']);
					...
					$PriceAmount->addAttribute('currencyID', $p['para_birimi']);
